...
class RobotCommander(Node):
    ...
    def ring_color_callback(self, msg):
        print(f"COLOR OF RING BEING SENT: {msg}")
...
def main(args=None):
    ...
    # for testing
    points = [[1.2, 0.99, 0.00],[-1.6, 1.22, 0.0],[-1.41, 4.36,-0.165],[-1.35, 3.17,-0.568],[1.9,3.04,0.57],[2.48,1.81,0.00247],[0.39,1.87,-0.207],[1.34,0.308,0.0582],[2.23,-1.78,-1],[3.27,-1.4,0.961],[1.14,-1.8,-1.0],[-0.16,-1.33,0.832]]
    # ,[-0.7, 1.42,-0.584] 3
    # [-0.464, 0.18, 0,00]
    # [1.5,-0.4,-0.069] --> 10
    # [2.23,-1.78,-1] --> 11
    ...
    marked_rings = []
    marked_poses = []
    # model = tf.keras.models.load_model('./src/dis_tutorial3/scripts/anomaly_detection_model.h5')
    ...
    #         print(f"Marked rings: {marked_rings}")
    #         print(f"RINGS FOUND: {rc.rings_found}")

    #         if rc.list_of_suggested_rings_1 and rc.list_of_suggested_rings_2 and len(rc.rings_found) >= 4: # since there are always 4 rings!
    #             arm_msg = String()
    ...
    # ------------------------LOOKING FOR CYLINDER MISSING????????---------------------------

    goal_pose = PoseStamped()
    goal_pose.header.frame_id = 'map'
    goal_pose.header.stamp = rc.get_clock().now().to_msg()
    goal_pose.pose.position.x = points[0][0]
    goal_pose.pose.position.y = points[0][1]
    goal_pose.pose.orientation = rc.YawToQuaternion(points[0][2])
    ...
    # WHEN detect_mona_liza finds real Mona Lisa it publishes a msg to commander that he found it
    # After that we destroy Node!
    marked_poses = []
